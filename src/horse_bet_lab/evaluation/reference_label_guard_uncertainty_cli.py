...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run bootstrap uncertainty diagnostics on the reference strategy with "
            "valid-selected global weak-label guards."
        ),
    )
    parser.add_argument("--config", type=Path, required=True, help="Path to TOML config.")
    args = parser.parse_args()

    result = run_reference_label_guard_uncertainty(
        load_reference_label_guard_uncertainty_config(args.config),
    )
    row = result.summary
    print(
