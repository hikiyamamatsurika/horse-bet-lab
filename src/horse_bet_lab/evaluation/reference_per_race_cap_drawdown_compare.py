...
class PerRaceCapDrawdownSummary:
    stake_variant: str
...
@dataclass(frozen=True)
class PerRaceCapDrawdownGroupSummary:
    stake_variant: str
    per_race_cap_stake: float
    initial_bankroll: float
    group_key: str
...
@dataclass(frozen=True)
class PerRaceCapDrawdownPathRow:
    stake_variant: str
    per_race_cap_stake: float
    initial_bankroll: float
    sequence: int
...
@dataclass(frozen=True)
class PerRaceCapDrawdownBootstrapRow:
    stake_variant: str
    per_race_cap_stake: float
    initial_bankroll: float
    iteration: int
...
@dataclass(frozen=True)
class ReferencePerRaceCapDrawdownCompareResult:
    output_dir: Path
    summaries: tuple[PerRaceCapDrawdownSummary, ...]
    yearly_summaries: tuple[PerRaceCapDrawdownGroupSummary, ...]
    monthly_summaries: tuple[PerRaceCapDrawdownGroupSummary, ...]
    equity_rows: tuple[PerRaceCapDrawdownPathRow, ...]
    bootstrap_rows: tuple[PerRaceCapDrawdownBootstrapRow, ...]


def run_reference_per_race_cap_drawdown_compare(
    config: ReferencePerRaceCapDrawdownCompareConfig,
) -> ReferencePerRaceCapDrawdownCompareResult:
    ...
    summaries: list[PerRaceCapDrawdownSummary] = []
    yearly_summaries: list[PerRaceCapDrawdownGroupSummary] = []
    monthly_summaries: list[PerRaceCapDrawdownGroupSummary] = []
    equity_rows: list[PerRaceCapDrawdownPathRow] = []
    bootstrap_rows: list[PerRaceCapDrawdownBootstrapRow] = []

    for initial_bankroll in config.initial_bankrolls:
        for per_race_cap_stake in config.per_race_cap_values:
            for stake_variant in (
                "capped_fractional_kelly_like_per_race_cap",
                "capped_fractional_kelly_like_per_race_cap_drawdown_reduction",
            ):
    ...
    result = ReferencePerRaceCapDrawdownCompareResult(
...
def build_bootstrap_rows(
    *,
    ...
    per_race_cap_stake: float,
    stake_variant: str,
) -> list[PerRaceCapDrawdownBootstrapRow]:
    return [
        PerRaceCapDrawdownBootstrapRow(
            stake_variant=stake_variant,
            per_race_cap_stake=per_race_cap_stake,
            initial_bankroll=initial_bankroll,
...
) -> tuple[PerRaceCapDrawdownGroupSummary, ...]:
    return tuple(
        PerRaceCapDrawdownGroupSummary(
            stake_variant=stake_variant,
            per_race_cap_stake=per_race_cap_stake,
            initial_bankroll=initial_bankroll,
...
) -> tuple[PerRaceCapDrawdownPathRow, ...]:
    return tuple(
        PerRaceCapDrawdownPathRow(
            stake_variant=stake_variant,
            per_race_cap_stake=per_race_cap_stake,
            initial_bankroll=initial_bankroll,
