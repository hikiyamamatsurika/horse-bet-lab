...
def test_within_band_regime_diff_outputs_expected_artifacts(tmp_path: Path) -> None:
    ...
    analysis_config_path = tmp_path / "within_band.toml"
    ...
    result = run_within_band_regime_diff_analysis(
        load_within_band_regime_diff_config(analysis_config_path),
    )

    assert (result.output_dir / "summary.csv").exists()
    assert (result.output_dir / "finish_position_distribution.csv").exists()
    assert (result.output_dir / "ranking_summary.csv").exists()
    assert (result.output_dir / "calibration_summary.csv").exists()
    assert (result.output_dir / "representative_examples.csv").exists()
    assert any(
        row.regime_label == "2023" and row.selection_status == "adopted"
        for row in result.summaries
    )
    assert any(
        row.regime_label == "2024_2025" and row.selection_status == "non_adopted"
        for row in result.summaries
    )
    assert any(row.metric == "edge" for row in result.calibration_rows)
    assert any(row.selection_status == "adopted" for row in result.ranking_rows)
