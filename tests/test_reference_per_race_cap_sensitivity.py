...
def test_reference_per_race_cap_sensitivity_loader(tmp_path: Path) -> None:
    ...
    config_path = tmp_path / "reference_per_race_cap_sensitivity.toml"
    config_path.write_text(
        "\n".join(
            [
                "[analysis]",
                "name = 'reference_per_race_cap_sensitivity_test'",
                f"reference_bankroll_simulation_config_path = '{bankroll_config}'",
                f"output_dir = '{tmp_path / 'output'}'",
                "initial_bankrolls = [10000, 30000]",
                "per_race_cap_values = [200, 300, 400]",
    ...
    config = load_reference_per_race_cap_sensitivity_config(config_path)
    assert config.initial_bankrolls == (10000.0, 30000.0)
    assert config.per_race_cap_values == (200.0, 300.0, 400.0)
