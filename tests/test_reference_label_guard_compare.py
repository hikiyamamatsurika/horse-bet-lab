...
def test_reference_label_guard_compare_outputs_expected_artifacts(tmp_path: Path) -> None:
    ...
    analysis_config_path = tmp_path / "reference_label_guard_compare.toml"
    ...
    result = run_reference_label_guard_compare(
        load_reference_label_guard_compare_config(analysis_config_path),
    )

    assert (result.output_dir / "candidate_summary.csv").exists()
    assert (result.output_dir / "candidate_test_rollup.csv").exists()
    ...
    assert any(row.group_key == "all_oos" for row in result.candidate_test_rollups)
