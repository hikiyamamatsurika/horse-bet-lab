...
def test_reference_stake_sizing_compare_outputs_expected_artifacts(tmp_path: Path) -> None:
    ...
    label_guard_config_path = tmp_path / "reference_label_guard_compare.toml"
    analysis_config_path = tmp_path / "stake_sizing.toml"
    ...
    label_guard_config_path.write_text(
        "\n".join(
            [
                "[analysis]",
    ...
    analysis_config_path.write_text(
        "\n".join(
            [
                ...
                f"reference_label_guard_compare_config_path = '{label_guard_config_path}'",
    ...
    result = run_reference_stake_sizing_compare(
        load_reference_stake_sizing_compare_config(analysis_config_path),
    )
    ...
    assert any(row.stake_variant == "flat_100" for row in result.summaries)
    assert any(
        row.stake_variant == "capped_fractional_kelly_like_per_day_cap"
        for row in result.summaries
    )
    assert any(row.group_key == "2023-02" for row in result.monthly_summaries)
