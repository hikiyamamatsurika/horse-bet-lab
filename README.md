...
cross-line の役割分担と比較ルールは [docs/cross_line_comparison_rules.md](/Users/matsurimbpblack/Library/Mobile Documents/com~apple~CloudDocs/codex_projects/horse-bet-lab/docs/cross_line_comparison_rules.md:1) に固定しています。`mainline place reference / single-win anchor module / wide research candidate / proxy-live rank aid` は役割が違うため、単位の違う ROI や score を直接並べず、比較可能性は [docs/cross_line_summary.csv](/Users/matsurimbpblack/Library/Mobile Documents/com~apple~CloudDocs/codex_projects/horse-bet-lab/docs/cross_line_summary.csv:1) を参照します。
...
- `HJC` / `OZ` は現在の ingest で staging 化しているが、dataset builder contract の主対象ではなく evaluation-side の result / market proxy source として扱う
- `OW` の fixed-length 読みは引き続き research 実装のままで、本線 ingest には持ち込んでいない
...
`wide_research_v7` は `single_win_anchor_module_v2` の結論を受けて、anchor 本体を `win_pred_top1` に固定し、`place_vs_win_gap / win_edge_plus_place_gap` は partner 側の補助 tilt としてだけ使う比較です。sample config は [configs/wide_research_v7_win_pred_anchor_gap_tilt_dual_market_logreg_lift.toml](/Users/matsurimbpblack/Library/Mobile Documents/com~apple~CloudDocs/codex_projects/horse-bet-lab/configs/wide_research_v7_win_pred_anchor_gap_tilt_dual_market_logreg_lift.toml:1) で、`place_edge / pred_times_place_basis / reference_mix` に gap tilt を足した 6 通りを比較します。実データの rolling OOS では最良が `win_pred_anchor_plus_gap_tilted_partner + v7_win_pred_top1_partner_place_edge_place_vs_win_gap` で `pair_count=5109 / hit_count=1222 / hit_rate=0.2392 / roi=0.8459 / total_profit=-78740 / window_win_count=2 / mean_roi=0.8409 / roi_std=0.1149` でした。`v3 baseline (roi=0.8647 / profit=-69100)` にも `v6 profit candidate (roi=0.8768 / profit=-62930)` にも届かず、gap 系は今回も anchor の主役ではなく partner 補助 signal に留めるのが妥当です。
...
- `SED` の `distance_m` は位置確認までで feature 用 contract には入れていない
- `win_odds` は `OZ.win_basis_odds` を carrier 候補として `jrdb_pre_race_market_staging` / `jrdb_win_market_snapshot_v1` へ先行移行した
- `popularity` は pre-race carrier 未確認のため、今回も `legacy_sed_only_non_mainline` として `SED` path に残している
- popularity carrier resolution spike の固定判断は `unresolved_keep_legacy_for_non-mainline_only`
  - JRDB 公開導線では `OZ` は前日情報系の基準オッズデータ、`SED` は成績情報系/成績速報データとして出ており、same-carrier confirmed にはできない
  - したがって `popularity` は mainline confirmed feature には上げず、legacy / research / auxiliary に限定する
- decision freeze として、`OZ.win_basis_odds` は legacy `SED.win_odds` の drop-in replacement としては mainline 採用しない
  - carrier migration evaluation は `rollback`
  - discrepancy audit は `hold_and_deeper_source_audit`
  - 最有力な理由は broad join collapse ではなく、semantic mismatch with a timing mismatch component
  - current hard-adopt baseline は維持し、follow-up は low-correlation race tail の narrow source audit に限定する
...
- `OZ` は upstream raw の race-level odds array を project parser で `race_key + horse_number` 粒度へ展開して扱う。`horse_number` と array position の対応は repo-validated な parser convention であり upstream-formalized ではなく、`win_basis_odds` / `place_basis_odds` も repo 内 operational name として使う
...
train / eval / backtest parity のための feature 入力契約は
[docs/spec/feature_contract_v1.md](/Users/matsurimbpblack/Library/Mobile Documents/com~apple~CloudDocs/codex_projects/horse-bet-lab/docs/spec/feature_contract_v1.md:1)
を正本として扱います。
...
- target: `target_value`
- `odds_only` では `win_odds` を `jrdb_win_market_snapshot_v1` から、`popularity` を `legacy_sed_only_non_mainline` path から読む
...
- `win_odds` は現在 `jrdb_win_market_snapshot_v1` を stable contract として読み、carrier row は `OZ.win_basis_odds` から project-owned staging に materialize している
- `popularity` は pre-race carrier 未確認のため `SED` legacy path のまま残している
- fixed decision は `unresolved_keep_legacy_for_non-mainline_only`
...
- この deeper compare では `threshold=0.00` が全 window で選ばれ、popularity band は主に `1-5`、win_odds band は主に `3-10` と `5-15` が残った。一方で standalone ROI は 1.0 を超えず、前段の単純 dual-market line (`ROI=0.9306`) も上回れなかったため、現時点の single-win は mainline の standalone 本線候補ではなく、`dual_market_for_win` を暫定 best とする `wide anchor` モジュール候補として整理するのが妥当。
- この整理では `data/artifacts/single_win_market_comparison_2024_2025/dual_market_for_win/backtest/anchor_module_reference.csv` と `.json` を downstream 参照用に残し、列は `race_key / horse_number / split / window_label / target_value / win_pred / win_market_prob / win_edge / place_basis_odds / place_market_prob` に固定する。これは後続の wide research で anchor signal を再利用するための参照出力であり、reference pack や proxy-live の mainline artifact と混同しない。
- `single_win_anchor_module_v2` は standalone で勝ち切るための延長ではなく、anchor 品質改善のために `win_pred / win_edge / place_vs_win_gap / win_edge_plus_place_gap` を比較する研究ラインである。2024-2025 rolling OOS では `win_pred` が `AUC=0.8400 / logloss=0.2057 / Brier=0.0583 / BET=5109 / 的中率=0.3427 / 回収率=0.8152 / 総利益=-94410 / ROI>1.0 ratio=0.0000 / ROI95%=[0.7823, 0.8533]` で最良、`place_vs_win_gap` と `win_edge_plus_place_gap` は market distortion の補助 signal としては保持価値がある一方、standalone anchor 置換には届いていない。
...
- 現時点の本線 reference pack は、`configs/reference_pack_dual_market_logreg_mainline_2023_2025.toml` で再現できる
  ...
  - BET logic only の frozen baseline は `guard_0_01_plus_proxy_domain_overlay`
  - status は `hard_adopt`
  - fallback は `no_bet_guard_stronger surcharge=0.01`
  - これは `no_bet_guard_stronger surcharge=0.01` を base にし、venue-code-based domain bucket `02 / 09` にだけ追加 surcharge を載せる最小 overlay
  - 前提となる formalization は次で固定する
    - `race_key` は upstream race identifier
    - `race_key[:2]` は upstream-defined `venue_code`
    - domain/group は upstream category ではなく、`venue_code` から project 側 mapping で導出する
  - historical artifact name の `proxy_domain` は互換性維持のため残しているが、意味としては `venue_code` 由来の project-owned bucket
  - `candidate_provisional` / `fallback_stable` は package 互換の legacy `run_mode` 名で、adopt status そのものではない
  - `formal_domain_mapping_confirmed=true` は venue_code formalization と project-owned mapping を repo 内で固定した状態を表す
  - behavior change ではなく baseline fixation であり、新しい BET logic は作っていない
  - frozen baseline の正式な source of truth は [docs/mainline_reference_runbook.md](/Users/matsurimbpblack/Library/Mobile%20Documents/com~apple~CloudDocs/codex_projects/horse-bet-lab/docs/mainline_reference_runbook.md)
  - `win_odds` carrier についても decision freeze を適用し、`OZ.win_basis_odds` は legacy `SED.win_odds` replacement としては採用しない
    - current hard-adopt baseline はそのまま維持する
    - `popularity` true pre-race carrier は引き続き unresolved
    - follow-up は suspicious races の narrow source audit と `popularity` carrier confirmation のみ
  - metrics snapshot:
    - candidate `guard_0_01_plus_proxy_domain_overlay`: `bet_count=161 / hit_count=100 / hit_rate=0.6211 / roi_multiple=1.1944 / total_profit=3130 / max_drawdown=630`
    - fallback `no_bet_guard_stronger surcharge=0.01`: `bet_count=171 / hit_count=102 / hit_rate=0.5965 / roi_multiple=1.1480 / total_profit=2530 / max_drawdown=740`
  - comparison では次を順に見る
    - `docs/mainline_reference_runbook.md`
    - `data/artifacts/bet_logic_only_dual_market_logreg_mainline_2023_2025/logic_status.csv`
    - `data/artifacts/bet_logic_only_dual_market_logreg_mainline_2023_2025/summary.csv`
    - `data/artifacts/bet_logic_only_dual_market_logreg_mainline_2023_2025/hard_adopt_decision.csv`
    - `final_instruction_package_summary`, `final_instruction_package_manifest`, `regression_gate_report`
  ...
  - current reading:
    - uncertainty は `race_date block bootstrap` を基準に読む
    - `week / month` block でも confidence の大勢は維持
    - valid-selected extra label guard の uplift は `race_internal_permutation` null を主読みにして小さめに読む
    - mainline は比較基準として固定し、古い数値や旧 CI は引用しない
    - short reading:
      - strategy 単体: `ROI 95% interval = 0.9940 - 1.2622`, `ROI > 1.0 ratio = 0.9670`
      - stateful (`bankroll=10000`, `per_race_cap=200`): `ROI 95% interval = 1.0034 - 1.2180`, `ROI > 1.0 ratio = 0.9790`
      - extra label guard: `race_internal_permutation` null では `observed ROI percentile = 0.921`, `observed - null median ROI = +0.0136`
  - config:
  ...
  - uncertainty bootstrap note:
    ...
    - `week / month` block sensitivity でも mainline の大勢は維持した
  - 読み方:
    ...
    - valid-selected extra label guard の uplift は `race_internal_permutation` null を主読みにして、以前より小さめに読む
    ...
    - 古い数値、旧 CI、旧 uplift は比較基準として再引用しない
...
- 注記:
  ...
  - `proxy_feature_gap_summary.json` の historical overlap は、public 複勝レンジ midpoint proxy と実 OZ のズレを見るための補助診断であり、proxy_live の読み方を保守的にするために使う。mean / median / quantile / 相対誤差が大きい場合は、proxy_live の順位解釈をより弱く扱う
  - 2025年 selected 6 races overlap では、`mean_abs_relative_error` は mean `0.800` / median `0.799` / p75 `0.910` / worst `1.030`、`spearman_rank_correlation` は mean `0.922` / median `0.914` / worst `0.897`、`top5_overlap_count` は mean `4.0` / worst `3`、`top3_overlap_count` は mean `2.67` / worst `2` だった。proxy_live は本線代替ではなく、順位の参考に限って弱く使う
  - 暫定 guardrail は `green`: `mean_abs_relative_error <= 0.799` かつ `spearman >= 0.914` かつ `top5_overlap >= 4` かつ `top3_overlap >= 3`、`yellow`: `mean_abs_relative_error <= 0.910` かつ `spearman >= 0.911` かつ `top5_overlap >= 4` かつ `top3_overlap >= 2`、`red`: それ以外。`red` は proxy_live を探索的な補助文脈としてしか使わない
